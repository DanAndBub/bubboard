...
export default function StatsBar({ stats }: StatsBarProps) {
  const scoreColor =
    stats.score >= 8 ? '#34d399' : stats.score >= 5 ? '#fbbf24' : '#f87171';
  ...
  return (
    <div className="border border-[#506880] rounded-xl bg-[#111827] overflow-hidden">
      <div className="grid grid-cols-3 sm:grid-cols-5 divide-x divide-[#506880]">
        {items.map(item => (
          <div
            key={item.label}
          ...
          >
            ...
            <div className="text-xs text-[#7a8a9b] text-center">{item.label}</div>
          </div>
        ))}
      </div>
    </div>
