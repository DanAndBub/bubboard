...
export default function HealthCheck({ report }: HealthCheckProps) {
  ...
  const scoreColor =
    report.score >= 8 ? '#34d399' : report.score >= 5 ? '#fbbf24' : '#f87171';
  const scoreLabel =
  ...
  return (
    <div className="rounded-xl border border-[#506880] bg-[#111827] overflow-hidden">
      {/* Header */}
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#506880]">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-[#f1f5f9] text-sm">Health Check</h3>
        </div>
        {/* Score */}
        <div className="flex items-center gap-3">
          <div className="text-right">
            ...
            <div className="text-xs text-[#7a8a9b]">{scoreLabel}</div>
          </div>
          {/* Score ring */}
          <div className="relative w-10 h-10">
            <svg className="w-10 h-10 -rotate-90" viewBox="0 0 36 36">
              <circle cx="18" cy="18" r="15" fill="none" stroke="#3a4e63" strokeWidth="3" />
              <circle
                cx="18" cy="18" r="15"
                fill="none"
                stroke={scoreColor}
        ...
        {report.items.map(item => (
          <div key={item.id} className="flex items-start gap-3 px-5 py-3 hover:bg-[#1a2235] transition-colors group">
            ...
            <div className="mt-0.5 shrink-0">
              {item.status === 'ok' ? (
                ...
                <div className="w-5 h-5 rounded-full bg-amber-500/15 border border-amber-500/30 flex items-center justify-center">
                  <svg className="w-3 h-3 text-[#fbbf24]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-mono text-xs font-medium text-[#f1f5f9]">{item.label}</span>
                <span className="text-xs text-[#7a8a9b]">—</span>
                <span className="text-xs text-[#b0bec9] truncate">{item.description}</span>
              </div>
              {item.status !== 'ok' && item.recommendation && (
                <p className="mt-1 text-xs text-[#7a8a9b] leading-relaxed">
                  💡 {item.recommendation}
                </p>
              )}
            </div>
